...
class Basic(unittest.TestCase):
    ...
    def test_raw(self):
        ...
        self.assertRaises(raw.BadSignatureError, raw.open, sig+msg, flip_bit(vk_s, in_byte=2))
        self.assertRaises(raw.BadSignatureError, raw.open, flip_bit(sig)+msg, vk_s)
        self.assertRaises(raw.BadSignatureError, raw.open, flip_bit(sig, in_byte=33)+msg, vk_s)
    ...
    def test_keypair(self):
        ...
        self.assertEqual(sk1.to_ascii(encoding="base64"), b"NDQ0NDQ0NDQ0NDQ0NDQ0NDQ0NDQ0NDQ0NDQ0NDQ0NDQ")
        self.assertEqual(vk1.to_ascii(encoding="base64"), b"6yzxO/euOl9hQWih+wknLTl3HsS4UjcngV5GbK+O4WM")
        sk2, vk2 = ed25519.create_keypair(entropy=not_so_random)
        self.assertEqual(sk1.to_ascii(encoding="base64"), sk2.to_ascii(encoding="base64"))
        self.assertEqual(vk1.to_ascii(encoding="base64"), vk2.to_ascii(encoding="base64"))


    def test_publickey(self):
        seed = unhexlify("4ba96b0b5303328c7405220598a587c4" + "acb06ed9a9601d149f85400195f1ec3d")
        sk = ed25519.SigningKey(seed)
        self.assertEqual(hexlify(sk.to_bytes()), (b"4ba96b0b5303328c7405220598a587c4" + b"acb06ed9a9601d149f85400195f1ec3d" + b"a66d161e090652b054740748f059f92a" + b"5b731f1c27b05571f6d942e4f8b7b264"))
        self.assertEqual(hexlify(sk.to_seed()), (b"4ba96b0b5303328c7405220598a587c4" + b"acb06ed9a9601d149f85400195f1ec3d"))
        self.assertRaises(ValueError, ed25519.SigningKey, b"wrong length")
        sk2 = ed25519.SigningKey(seed)
        self.assertEqual(sk, sk2)

    def test_OOP(self):
        sk_s = unhexlify("4ba96b0b5303328c7405220598a587c4" + "acb06ed9a9601d149f85400195f1ec3d" + "a66d161e090652b054740748f059f92a" + "5b731f1c27b05571f6d942e4f8b7b264")
        sk = ed25519.SigningKey(sk_s)
        self.assertEqual(len(sk.to_bytes()), 64)
        self.assertEqual(sk.to_bytes(), sk_s)

        sk2_seed = unhexlify("4ba96b0b5303328c7405220598a587c4" + "acb06ed9a9601d149f85400195f1ec3d")
        sk2 = ed25519.SigningKey(sk2_seed)
        self.assertEqual(sk2.to_bytes(), sk.to_bytes())

        vk = sk.get_verifying_key()
        self.assertEqual(len(vk.to_bytes()), 32)
        exp_vks = unhexlify("a66d161e090652b054740748f059f92a" + "5b731f1c27b05571f6d942e4f8b7b264")
        self.assertEqual(vk.to_bytes(), exp_vks)
        self.assertEqual(ed25519.VerifyingKey(vk.to_bytes()), vk)
        msg = b"hello world"
        sig = sk.sign(msg)
        self.assertEqual(len(sig), 64)
        exp_sig = unhexlify("6eaffe94f2972b35158b6aaa9b69c1da" + "97f0896aca29c41b1dd7b32e6c9e2ff6" + "76fc8d8b034709cdcc37d8aeb86bebfb" + "173ace3c319e211ea1d7e8d8884c1808")
        self.assertEqual(sig, exp_sig)
    ...
    def test_object_identity(self):
        sk1_s = unhexlify("ef32972ae3f1252a5aa1395347ea008c" + "bd2fed0773a4ea45e2d2d06c8cf8fbd4" + "c024601a9c5b854fb100ff3116cf4f22" + "a311565f027391cb49d3bbe11c44399d")
        sk2_s = unhexlify("3d550c158900b4c2922b6656d2f80572" + "89de4ee65043745179685ae7d29b944d" + "672b8a2cb23f9e75e1d46ce249cd9c04" + "68f816f1c734a102822b60e18b41eacd")
    ...
    def test_prefix(self):
        ...
        self.assertRaises(ed25519.BadPrefixError, ed25519.SigningKey, p, prefix=b"WRONG-")
        ...
        self.assertRaises(ed25519.BadPrefixError, ed25519.VerifyingKey, p, prefix=b"WRONG-")
        ...
        self.assertRaises(ed25519.BadPrefixError, vk1.verify, p, b"msg", prefix=b"WRONG-")
